...
1. Reply to every inline thread before you merge, and name what changed. One
   sentence is enough.
1. Resolve the thread after you post that reply and the answer is applied or
   accepted: the fix is pushed, or the reviewer accepts an answer that needs no
   change.
1. When you fixed it in another PR, the Problem, or a follow-up issue, say which
   one and give its number.
1. When you disagree, reply with why and leave the thread open. The reviewer
   decides, per [DEV-410](./DEV-410.md).
